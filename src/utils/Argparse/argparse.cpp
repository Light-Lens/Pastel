...
std::vector<argparse::parsed_argument> argparse::parse(const std::vector<std::string>& args)
{
    ...
    {
        std::string lowercase_arg = utils::lowercase(args[i]);
        ...
        {
            std::string str_missing_arg_list = "Missing required argument(s): ";
            for (int i = 0; i < missing_arg_list.size(); ++i)
            {
                str_missing_arg_list += missing_arg_list[i];
                if (i < missing_arg_list.size() - 1)
                    str_missing_arg_list += ", ";
            }
